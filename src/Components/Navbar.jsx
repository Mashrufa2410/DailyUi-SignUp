...
import { auth } from "../firebase.init"; // Correct import
...
const Navbar = () => {
  ...
  return (
    <nav className="navbar bg-base-100 shadow-lg px-5 md:px-10 fixed top-0 left-0 w-full z-50">
      ...
      <div className="navbar-end flex items-center gap-3">
        {user ? (
          <div className="flex items-center gap-3">
            ...
            <button
              className="py-1 px-3 lg:py-2 lg:px-4 bg-black flex text-white hover:bg-gray-800"
      ...
      <div className="navbar-end lg:hidden">
        <button className="btn btn-ghost" onClick={() => setMenuOpen((prev) => !prev)} aria-label="Toggle menu">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 12h16M4 18h16" />
          </svg>
        </button>
        {menuOpen && (
          <ul className="absolute right-5 top-full mt-2 p-2 shadow-lg bg-base-100 rounded-lg w-48">
            <li>
              <NavLink to="/" className={({ isActive }) => (isActive ? "active-link" : "")}>
                Home
              </NavLink>
